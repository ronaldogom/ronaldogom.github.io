...
GLdouble wix, wiy, wiz;  /*  coordenadas no mundo real: x, y, z  */
...
void proximidade(){
  ...
  for(i=0; i<4; i++){
    for(j=0; j<4; j++){
      tam = 
      (wx-vertices[i][j][0])*(wx-vertices[i][j][0])+
      (wx-vertices[i][j][1])*(wx-vertices[i][j][1])+
      (wx-vertices[i][j][2])*(wx-vertices[i][j][2]);
...
void motion(int x, int y){
    ...
    gluUnProject ((GLdouble) x, (GLdouble) yreal, 0.0, matrizModelview, matrizProjecao, matrizViewport, &wx,   &wy,  &wz);
    gluUnProject ((GLdouble) x, (GLdouble) yreal, 1.0, matrizModelview, matrizProjecao, matrizViewport, &wix, &wiy, &wiz);

 	wix -= wx;
    wiy -= wy;
    wiz -= wz;

    vertices[verticeCorrente][0][0]=wix;
    vertices[verticeCorrente][1][1]=wiy;
    vertices[verticeCorrente][2][2]=wiz;
